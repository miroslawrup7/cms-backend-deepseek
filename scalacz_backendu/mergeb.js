...
function readFilesRecursively(p, excludePaths, result = []) {
  ...
  if (stats.isFile()) {
    ...
    const files = fs.readdirSync(p).sort();
    files.forEach((file) =>
      readFilesRecursively(path.join(p, file), excludePaths, result),
    );
  }
  return result;
}
...
function isTextFile(filePath) {
  ...
  if (
    alwaysTextFiles.some(
      (textFile) =>
        fileName === textFile.toLowerCase() ||
        fileName.startsWith(textFile.toLowerCase() + '.'),
    )
  ) {
    return true;
  }
  ...
  for (const line of lines) {
    ...
    if (
      ...
      const outputFile = path.join(
        process.cwd(),
        `${outputName}_v${projectVersion}_part${currentPart}.txt`,
      );
...
const fileVersions = fs.existsSync(fileVersionsPath)
  ? JSON.parse(fs.readFileSync(fileVersionsPath, 'utf-8'))
  : {};

const projectVersionData = fs.existsSync(versionFile)
  ? JSON.parse(fs.readFileSync(versionFile, 'utf-8'))
  : { version: 0, structureHash: '' };
...
function mergeFiles(excludePaths, outputName) {
  ...
  const previousOutputFile = path.join(
    process.cwd(),
    `${outputName}_v${projectVersionData.version}.txt`,
  );
  const isFirstRun =
    projectVersionData.version === 0 || !fs.existsSync(previousOutputFile);
  ...
  allFiles.forEach((filePath) => {
    ...
    const hash = textFile
      ? md5Hash(Buffer.from(content, 'utf-8'))
      : md5Hash(fs.readFileSync(filePath));
    ...
    if (!fileVersions[filePath] || fileVersions[filePath].hash !== hash) {
      ...
      if (textFile) {
        ...
        fs.writeFileSync(
          histFile,
          '<plik binarny, nie zapisano treści>',
          'utf-8',
        );
      }
    }
  ...
  fs.writeFileSync(
    versionFile,
    JSON.stringify(
      { version: projectVersion, structureHash: currentHash },
      null,
      2,
    ),
  );
